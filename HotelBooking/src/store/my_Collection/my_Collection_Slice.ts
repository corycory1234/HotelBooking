...
const my_Collection_Slice = createSlice({
  ...
  reducers: {
    // 1. 空陣列 > 更新已收藏之陣列
    update_My_Collection: (state, action: PayloadAction<add_Hotel_Detail_Interface []>) => {
      state.my_Collection_List = action.payload
    },

    // 3. 新增收藏飯店
    add_My_Collection: (state, action: PayloadAction<add_Hotel_Detail_Interface>) => {
      ...
      state.my_Collection_List.unshift(the_Collected_Hotel);
    },

    // 4. 刪除收藏飯店
    delete_My_Collection: (state, action: PayloadAction<add_Hotel_Detail_Interface>) => {
      const the_Deleted_Hotel = action.payload;
      state.my_Collection_List = state.my_Collection_List.map((item) => {
        if(item.hotel_Id === the_Deleted_Hotel.hotel_Id) {
          return {...item, isCollected: false}
        };
        return item;
      });
      state.my_Collection_List = state.my_Collection_List.filter((item) => item.hotel_Id !== the_Deleted_Hotel.hotel_Id);
    }
