...
interface Hotel_Card_Interface {
  redux_Hotel_Detail: Hotel_Detail_Interface | undefined
};

export default function Swiper_Hotel_Detail({redux_Hotel_Detail}: Hotel_Card_Interface) {
  ...
  return <>
    ...
    <div className="">
      <Swiper slidesPerView={1}
        ...
        {redux_Hotel_Detail?.images.map((img, index) => {
          return <SwiperSlide key={index}>
            <div className="relative">
              ...
              <p className="absolute bottom-2 right-8 text-white text-sm">
                {(index < 10 ? "0" + (index + 1) : index + 1) + "/" + (redux_Hotel_Detail.images.length < 10 ? "0" + redux_Hotel_Detail.images.length : redux_Hotel_Detail.images.length)}
              </p>
