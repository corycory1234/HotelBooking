  ...
  return <>
    <div className="flex flex-col gap-2">
      {redux_Hotel_Room_Type.map((room) => {
        ...
        <div className="flex flex-col p-2 border border-gray rounded">
          
          {/** 房型價錢、住幾晚、人數 */}
          <div className="flex flex-col">
            <p className="font-bold text-lg">{room.price}</p>
            <p className="text-gray text-sm">{nights + " Nights"} | {redux_Fomr_Search.adult + " Adults"} | {redux_Fomr_Search.child + " Childs"}</p>
          </div>
          {/** 房型價錢、住幾晚、人數 */}

          {/** Book按鈕 */}
          <div className="flex justify-between items-center">
            <p className={`basis-1/2 ${room.availability <=3 ? 'text-red' : 'text-primary'}`}>{room.availability + " Rooms Left"}</p>
            <button className="basis-1/2 bg-primary p-2 rounded">Book</button>
          </div>
          {/** Book按鈕 */}
