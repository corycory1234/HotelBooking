...
export default function Edit_Hotel_List_Modal ({the_Editing_Hotel, onClose}: Editing_Hotel_Interface) {
  ...
  // 2. 本地 formData, 管理整個<form>、<input>、<select>等等
  const [formData, set_FomrData] = useState<add_Hotel_Detail_Interface | null>(null);

  // 3. 如果拿到 the_Editing_Hotel(有值時), 就{展開運算子}, 帶入給 formData
  useEffect(() => {
  ...
  // 5. <input> 或 <select> 當中的 onChange事件，其可以改變值, 因為太多<input>, 統一寫成函式, 再綁onChange
  const handle_Change = (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    if(!formData) return;
    const {name, value} = event.target;
    console.log(name, "有甚麼name");
    console.log(value, "有甚麼值");

    // 5.1 如果不是拿「設施陣列」數據, 才走以下
    ...
    // 5.2 以下為 "facilities" checkbox 的處理 
    // 5.3 先判斷 target 是否為 <input type="checkbox"> 才能取 'checked'
  ...
  // 6. 第2種, 搭配 <input type="checkbox"> 寫法, 這樣 event.target.checked就可以找到
  // 6.1 上面寫法找不到, 是因為吃太多ChangeEvent...
  const handle_Checkbox_Change = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // 確保 facility_List 是陣列，如果是 null 就給它空陣列
    let new_Facilities = formData.facility_List ?? [];

    // 6.4 若打勾，加入該 facility
    if(checked) {
      if(!new_Facilities.includes(facility_Value)) {
        new_Facilities = [...new_Facilities, facility_Value]
      }
    } else {
      // 6.5 若取消勾選，移除該 facility
      new_Facilities = new_Facilities.filter((item) => item !== facility_Value)
    };

    // 6.6 最後更新 formData, 主要針對 facility_List 更新
    set_FomrData({
      ...formData,
      facility_List: new_Facilities
    })
  ...
  // 8. review_List: [{留言1},{留言2}] 之 onChange 函式
  ...
  const handle_Room_Type_Change = (index: number, field: keyof add_Hotel_Room_Type_Interface, newValue: any) => {
    if(!formData) return;
    // 10.1 拷貝原先的 roomType_List
    const old_Room_Types = formData.roomType_List ?? [];
    // 10.2 修改指定 房型 (帶入index做辨認)
  ...
  const add_Room_Type = () => {
    ...
    // 9.4 新房型陣列, 展開運算子舊房型列表, 再於[]最後面補上「新房型-物件」
