...
class HotelController {
    ...
    async getHotels(req: Request, res: Response) {
        try {
            ...
            if (!page || isNaN(page)) {
                return res
                    .status(400)
                    .json(ApiResponse.error("頁碼(page)為必填參數"));
            }
            ...
            if (minPrice !== undefined && minPrice <= 0) {
                return res
                    .status(400)
                    .json(ApiResponse.error("最低價格必須大於 0"));
            }

            if (maxPrice !== undefined && maxPrice <= 0) {
                return res
                    .status(400)
                    .json(ApiResponse.error("最高價格必須大於 0"));
            }
            ...
            const searchParams: SearchHotelsParams = {
                ...
                facilities: req.query.facilities
                    ? (req.query.facilities as string)
                          .split(",")
                          .filter((f) => f.trim())
                    : undefined,
                bed_Types: req.query.bedTypes
                    ? (req.query.bedTypes as string)
                          .split(",")
                          .filter((b) => b.trim())
                    : undefined,
            };
    ...
    async uploadRoomTypeImages(req: Request, res: Response) {
        try {
            ...
            if (!files || files.length === 0) {
                return res
                    .status(400)
                    .json(ApiResponse.error("請選擇要上傳的照片"));
            }

            const results = await hotelService.uploadRoomTypeImages(
                roomTypeId,
                files,
                token
            );
    ...
    async deleteHotelImages(req: Request, res: Response) {
        try {
            ...
            if (!Array.isArray(imageUrls) || imageUrls.length === 0) {
                return res
                    .status(400)
                    .json(ApiResponse.error("請提供要刪除的照片 URL"));
            }
    ...
    async deleteRoomTypeImages(req: Request, res: Response) {
        try {
            ...
            await hotelService.deleteRoomTypeImages(
                roomTypeId,
                imageUrls,
                token
            );
    ...
    async uploadHotelImages(req: Request, res: Response) {
        try {
            ...
            const results = await hotelService.uploadHotelImages(
                hotelId,
                files,
                token
            );
